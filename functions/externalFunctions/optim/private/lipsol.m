...
%   $Revision: 1.1.6.3 $ $Date: 2007/12/10 21:50:33 $
...
if ~isempty(A)
    ...
    if (Dense_cols_exist)
        Rinf = [];
        [x,Sherman_OK,residual_OK,Mdense,Rinf] = ...
            ShermanSolve(P,U,full(b),1,Sherman_OK,Mdense,perm,Rinf);
        % if the Sherman-Morrison formula is not satisfactory calculate with \
        % instead of pcg.
        if ~residual_OK
            % Permutations might not be necessary here?
            x(perm) = (P(perm,perm)+U(perm,:)*U(perm,:)')\b(perm);
        end
        if ~Sherman_OK
            useNormalForm = false;
        end
   ...
   while (iter <= maxiter)
       ...
       if (iter > 0)
           ...
           if (trerror < tol)
               ...
               if (iter > 2)
                   ...
                   if (any(blowup))
                       if (useNormalForm)
                           % If a blowup is detected when using Normal Formulation, 
               ...
               if (iter > nstall)
                  ...
                  if (any(stall))
                      if (useNormalForm)
                           ...
                           % Formulation.This is less effective than the switch 
                           % for the blowup case above.
                          useNormalForm = false;
                          x = xc;
                          y = yc;
                          z = zc;
      ...
      [dx,dy,dz,ds,dw,ldlFactorUptodate,useNormalForm,Sherman_OK,Mdense,Rinf,L,Dg,Pr,Sc] = ...
         ...
         Ubounds_exist,Mdense,perm,Rinf,L,Dg,Pr,Sc);
      ...
      if ((tau0*ap < 1) || (tau0*ad < 1))
         ...
         [dx2,dy2,dz2,ds2,dw2,ldlFactorUptodate,useNormalForm,Sherman_OK,Mdense,Rinf,L,Dg,Pr,Sc] = ...
            direction(A,P,U,sparse(m,1),sparse(n,1),sparse(n,1),...
            Rxz,Rsw,vmid,rvmid,xn1,sn1,z,w,mu,m,n,2,ldlFactorUptodate,useNormalForm, ...
            Sherman_OK,Dense_cols_exist,Ubounds_exist,Mdense,perm,Rinf,L,Dg,Pr,Sc);
...
function [dx,dy,dz,ds,dw,ldlFactorUptodate,useNormalForm,Sherman_OK,Mdense,Rinf,L,Dg,Pr,Sc] = ...
...
if ~(useNormalForm) % use Augmented Formulation instead.
    if (flag == 1) || ((flag == 2) && ~ldlFactorUptodate)
        ...
        [L,Dg,Pr,Sc] = ldl([-rvmid A';A sparse(m,m)],'vector');
